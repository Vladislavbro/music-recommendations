...
(`research/grouprec/aggregators/base.py`, см. CLAUDE.md §4).
...
from grouprec.eval.metrics import ndcg_from_ranking
...
class SessionMetrics:
    ...
    def reset(self) -> None:
        self._acc = {m: {"n": 0, "sums": {k: 0.0 for k in METRIC_KEYS}} for m in METRICS_METHODS}
...
def _score(
    method: str, members: list[int], candidates: np.ndarray, per_user: np.ndarray
) -> np.ndarray:
...
def recommend(user_ids: list[int], method: str, top_n: int) -> dict:
    ...
    tracks = [
        {"track_id": int(candidates[i]), "score": round(float(scores[i]), 4)} for i in top_idx
    ]
...
def compute_group_metrics(members: list[int]) -> dict | None:
    """NDCG@K и coverage@K для METRICS_METHODS на одной группе.

    Ground-truth: `targets = union(test_listens членов) ∩ C_G` — ровно контракт
    офлайн-eval (`research/grouprec/eval/group_eval.py`). Если targets пуст → None (группа не
    ...
    tgt_parts = [
        STORE.test_targets[u]
        for u in members
        if u in STORE.test_targets and len(STORE.test_targets[u])
    ]
    if not tgt_parts:
        return None
