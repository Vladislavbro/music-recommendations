...
from src.eval.group_eval import (  # noqa: E402
    GroupSample,
    build_group_samples,
...
def test_build_group_samples_union_basic():
    ...
    samples, stats = build_group_samples(
        [[1, 2]], user_topk, test_tgt, ground_truth="union"
    )
...
def test_build_group_samples_intersection():
    ...
    samples, _ = build_group_samples(
        [[1, 2]], user_topk, test_tgt, ground_truth="intersection"
    )
    assert len(samples) == 1
...
def test_build_group_samples_drops_empty_target():
    ...
    samples, stats = build_group_samples(
        [[1, 2]], user_topk, test_tgt, drop_empty=True
    )
    assert samples == []
    assert stats["n_dropped_empty_targets"] == 1
    # And without drop:
    samples_keep, stats_keep = build_group_samples(
        [[1, 2]], user_topk, test_tgt, drop_empty=False
    )
...
def test_build_group_samples_drops_missing_member():
    ...
    samples, stats = build_group_samples(
        [[1, 2]], user_topk, test_tgt, drop_missing_member=True
    )
    assert samples == []
...
def test_build_group_samples_invalid_ground_truth():
    try:
        build_group_samples([[1, 2]], {1: np.array([10]), 2: np.array([20])}, {}, ground_truth="weird")  # type: ignore[arg-type]
    except ValueError:
        return
