...
`research/grouprec/aggregators/base.py`, путь загрузки/сборки входов скопирован из
`demo/club/backend/server.py`). Скорер заморожен и здесь НЕ запускается: его
...
Запуск:
    python research/tests/bench_inference.py                 # реальные артефакты
    python research/tests/bench_inference.py --runs 500       # больше прогонов
    python research/tests/bench_inference.py --out logs/...    # сохранить markdown-табличку
...
def sample_groups(store: Store, rng, n_per_size: int) -> dict:
    ...
    groups = {}
    for size in (2, 3, 4, 5):
        groups[size] = [list(rng.choice(uids, size=size, replace=False)) for _ in range(n_per_size)]
    return groups
...
def main() -> int:
    ...
    out()
    out(
        "> online ≪ full: скорер (заморожен) и аудио — амортизированная офлайн-цена; "
            ...
            for members in groups[size]:
                ...
                all_samples.extend(
                    time_forward(
                        model,
                        kwargs,
                        runs=max(1, args.runs // args.groups_per_size),
    ...
    out()
    out(
        "> |C_G| = размер union(top-K членов); латентность растёт с числом кандидатов, "
