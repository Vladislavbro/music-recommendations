...
class GSASRec(nn.Module):
    ...
    def forward(self, seq: torch.LongTensor) -> torch.Tensor:
        ...
        attn_mask = (
            attn_mask.unsqueeze(1).expand(B, self.n_heads, L, L).reshape(B * self.n_heads, L, L)
        )

        h = self.encoder(x, mask=attn_mask)
        return h

    def score(self, seq: torch.LongTensor, candidates: torch.LongTensor) -> torch.Tensor:
