...
class GroupIM(GroupAggregator):
    ...
    def __init__(
        ...
        self._cached_member_emb: torch.Tensor | None = None
        self._cached_group_repr: torch.Tensor | None = None
        self._cached_group_mask: torch.Tensor | None = None
    ...
    def mi_loss(
        ...
        group_mask: torch.Tensor | None = None,  # [B, G], True — реальный член
        neg_group_repr: torch.Tensor | None = None,  # [B, d]; по умолчанию — batch-shift
    ) -> torch.Tensor:
    ...
    def regularization_loss(
        ...
        """
        del batch
        if self._cached_member_emb is None or self._cached_group_repr is None:
