...
def build_group_samples(
    ...
    for members in groups:
        ...
        if ground_truth == "union":
            ...
            raw_targets = _intersection(tgt_arrays) if len(tgt_arrays) == len(member_tuple) else np.empty(0, dtype=np.int64)
...
def evaluate_aggregator_scores(
    ...
    if len(samples) != len(group_scores):
        raise ValueError(
            f"len(samples)={len(samples)} != len(group_scores)={len(group_scores)}"
        )
    k_list = list(k_list)
    n = len(samples)
    ...
    for i, (sample, scores) in enumerate(zip(samples, group_scores)):
...
def topk_from_score_cache(scores_df) -> dict[int, np.ndarray]:
    ...
    if not required.issubset(scores_df.columns):
        raise ValueError(
            f"scores_df must have columns {required}, got {set(scores_df.columns)}"
        )
...
def test_targets_from_df(test_df) -> dict[int, np.ndarray]:
    ...
    if not required.issubset(test_df.columns):
        raise ValueError(
            f"test_df must have columns {required}, got {set(test_df.columns)}"
        )
    out: dict[int, np.ndarray] = {}
