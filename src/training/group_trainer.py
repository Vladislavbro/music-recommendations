...
class GroupEvalDataset(Dataset):
    ...
    def __init__(
        ...
        item_audio: Optional[np.ndarray] = None,
        user_profiles: Optional[np.ndarray] = None,
        uid_to_row: Optional[Mapping[int, int]] = None,
...
class GroupTrainDataset(GroupEvalDataset):
    ...
    def __init__(
        ...
        self._neg_weights: list[Optional[np.ndarray]] = []
...
class GroupAggregatorTrainer:
    ...
    def fit(
        ...
        for epoch in range(cfg.n_epochs):
            ...
            if verbose:
                extra = " ".join(
                    f"val_NDCG@{k}={val_metrics[f'NDCG@{k}']:.4f}" for k in cfg.eval_k
                )
