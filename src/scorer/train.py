...
from .gsasrec import GSASRec, PAD_IDX
...
def train_gsasrec(
    ...
    with open(metrics_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "train_loss", "val_ndcg@%d" % cfg.eval_k, "epoch_seconds"])
    ...
    for epoch in range(cfg.n_epochs):
        ...
        for step, batch in enumerate(loader):
            ...
            loss = gbce_loss(pos_logits, neg_logits, n_items=cfg.n_items, n_neg=cfg.n_neg, t=cfg.gbce_t)
            ...
            if cfg.log_every_steps and (step + 1) % cfg.log_every_steps == 0:
                print(f"  epoch {epoch} step {step+1}/{len(loader)} loss={loss_sum/n_batches:.4f}")
        ...
        dt = time.time() - t0
        print(f"epoch {epoch}: loss={avg_loss:.4f} val_ndcg@{cfg.eval_k}={val_ndcg:.4f} time={dt:.1f}s")
